...
MAX_REQUEST_SIZE=396  # Ukuran batas yang telah ditentukan
URL="http://12c7.0.0.1:8080"  # Ubah dengan URL server yang sesuai
...
response=$(curl -s -o /dev/null -w "%{http_code}" -X GET --data "$PAYLOAD" $URL)
